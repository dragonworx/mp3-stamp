    ...
    if (config.cover) {
        Object.keys(config.cover).forEach(key => {
            ...
            if (typeof config.cover[key] === 'string') {
                ...
                console.log(cli.cyan(`✔️  Writing "${key}" cover -> ${path.relative('.', coverPath)}`));
            }
        });
    }
}
...
function writeFile() {
    ...
    try {
        ...
        console.log(cli.bold.green(`Done.`));
    } catch (e) {
...
async function main() {
    ...
    if (state.create) {
        ...
        console.log(cli.green(`"${path.basename(mp3Path)}.json" successfully created.`));
        process.exit(0);
    }
